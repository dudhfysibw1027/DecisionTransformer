...
class ActorCriticTransformer(TrajectoryModel):
    """
    ...
    def __init__(
        ...
        self.predict_mean = nn.Linear(hidden_size, self.act_dim)
        self.predict_log_std = nn.Linear(hidden_size, self.act_dim)
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None):
        ...
        mean_preds = self.predict_mean(x[:, 1])
        log_std_preds = self.predict_log_std(x[:, 1])
        std_preds = torch.exp(log_std_preds)

        return state_preds, (mean_preds, std_preds), return_preds
